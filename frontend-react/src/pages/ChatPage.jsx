...
export default function ChatPage() {
  ...
  function sendMessage() {
    ...
    socket.emit('send_message', { room: currentRoom, senderId: user.id, text });
    setMsgText('');
              ...
              return (
                <div key={conv.other._id} className={`conversation-item${isActive ? ' active' : ''}`} onClick={() => openChat(conv.other._id)}>
                  <div className="avatar" style={{ width: 42, height: 42, fontSize: '1rem', flexShrink: 0, background: 'linear-gradient(135deg,var(--blue),var(--purple))' }}>
                    {conv.other.avatar ? <img src={conv.other.avatar} style={{ width: 42, height: 42, borderRadius: '50%', objectFit: 'cover' }} /> : conv.other.name?.charAt(0)}
                  </div>
        ...
        {!currentOther ? (
          ...
          <>
            <div className="chat-header">
              <button className="btn btn-ghost btn-sm" onClick={() => setMobileOpen(false)} style={{ display: 'none' }} id="backBtn">← Назад</button>
              <div className="avatar" style={{ width: 40, height: 40, fontSize: '1rem', background: 'linear-gradient(135deg,var(--blue),var(--purple))' }}>
                {currentOther.avatar ? <img src={currentOther.avatar} style={{ width: 40, height: 40, borderRadius: '50%', objectFit: 'cover' }} /> : currentOther.name?.charAt(0)}
              </div>
              <div>
                <div style={{ fontWeight: 600 }}>{currentOther.name}</div>
                <div style={{ fontSize: '.75rem', color: 'var(--text3)' }}>{currentOther.role === 'client' ? '📋 Заказчик' : '💼 Фрилансер'}</div>
              </div>
            </div>
                ...
                : messages.map((msg, i) => {
                  ...
                  return (
                    <div key={i} className={`message-bubble ${isOwn ? 'own' : 'other'}`}>
                      {!isOwn && <div style={{ fontSize: '.75rem', color: 'var(--text3)', padding: '0 .25rem .1rem' }}>{msg.sender?.name}</div>}
                      <div className="bubble-text">{msg.text}</div>
                      <div className="bubble-meta">{new Date(msg.createdAt).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}</div>
                    </div>
                  );
                })
              }
            ...
            <div className="chat-input-area">
              <textarea ref={inputRef} className="chat-input" placeholder="Написать сообщение..." rows={1}
                ...
                style={{ resize: 'none' }} />
              <button className="btn btn-primary" onClick={sendMessage} style={{ borderRadius: '50%', width: 44, height: 44, padding: 0, flexShrink: 0 }}>➤</button>
            </div>
          </>
        )}
      </div>
