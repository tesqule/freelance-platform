...
async function loadProfile() {
  ...
  try {
    ...
    if (headerEl) {
      headerEl.innerHTML = `
        <div class="card" style="padding:1.5rem;border-radius:var(--radius-lg);display:flex;align-items:center;gap:1.25rem;flex-wrap:wrap;">
          <div class="avatar" style="width:76px;height:76px;font-size:1.8rem;flex-shrink:0;box-shadow:0 4px 20px rgba(79,110,247,.3);">
          ...
          <div style="flex:1;min-width:0;">
            <div style="font-family:var(--font-display);font-size:1.3rem;font-weight:700;letter-spacing:-.03em;">${profile.name}</div>
            <div style="margin:.3rem 0;">
              <span class="profile-role">${profile.role === 'client' ? '📋 Заказчик' : '💼 Фрилансер'}</span>
            </div>
          ...
          <div style="display:flex;gap:1.25rem;flex-shrink:0;">
            <div style="text-align:center;">
              <div style="font-family:var(--font-display);font-size:1.4rem;font-weight:800;color:var(--amber);">★ ${(profile.rating||0).toFixed(1)}</div>
              <div style="font-size:.7rem;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;">Рейтинг</div>
            </div>
            <div style="text-align:center;">
              <div style="font-family:var(--font-display);font-size:1.4rem;font-weight:800;color:var(--green);">${profile.completedTasks||0}</div>
              <div style="font-size:.7rem;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;">Выполнено</div>
            </div>
          </div>
        </div>`;
    }
    ...
    // Обновляем превью навыков если есть
...
async function loadPortfolio() {
  ...
  try {
    ...
    grid.innerHTML = items.map(item => `
      <div style="background:var(--card);border:1px solid var(--border);border-radius:var(--radius-lg);overflow:hidden;transition:all .22s;"
        ...
        <div style="width:100%;height:155px;background:var(--bg3);display:flex;align-items:center;justify-content:center;font-size:2.5rem;overflow:hidden;">
          ${item.imageUrl
            ? `<img src="${item.imageUrl}" style="width:100%;height:155px;object-fit:cover;" onerror="this.parentElement.innerHTML='${catEmojis[item.category]||'🔧'}';">`
            : catEmojis[item.category]||'🔧'}
        </div>
        <div style="padding:1rem;">
          <div style="font-weight:700;font-size:.95rem;margin-bottom:.3rem;">${item.title}</div>
          ${item.description ? `<div style="font-size:.8rem;color:var(--text2);line-height:1.6;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;margin-bottom:.5rem;">${item.description}</div>` : ''}
          ${item.skills?.length ? `
    ...
    const grid = document.getElementById('portfolioGrid');
    if (grid) grid.innerHTML = '<p style="color:var(--text2);padding:1rem;">Ошибка загрузки</p>';
  }
}
...
function closePortfolioModal() {
  ...
  if (titleEl) titleEl.textContent = 'Добавить работу';
  ['pTitle','pDesc','pImageUrl','pProjectUrl','pSkills'].forEach(id => {
...
async function editPortfolioItem(id) {
  try {
    ...
    const titleEl = document.getElementById('portfolioModalTitle');
    if (titleEl) titleEl.textContent = 'Редактировать работу';
...
async function loadReviews() {
  ...
  try {
    ...
    if (!reviews.length) {
      ...
      listEl.innerHTML = `
        <div class="empty-state" style="padding:2rem 0;">
          <div class="empty-icon">⭐</div>
          <p>Выполняйте задания чтобы получать отзывы</p>
        </div>`;
      return;
    }
    ...
    if (starsEl) starsEl.innerHTML  =
      `<span style="color:var(--amber)">${'★'.repeat(Math.round(avg))}</span>` +
      `<span style="color:var(--text3)">${'★'.repeat(5-Math.round(avg))}</span>`;
      ...
      <div style="padding:1.1rem 0;border-bottom:1px solid var(--border);">
        <div style="display:flex;gap:.75rem;">
          <div class="avatar" style="width:38px;height:38px;font-size:.9rem;flex-shrink:0;">
            ${r.reviewer.avatar
              ? `<img src="${r.reviewer.avatar}" style="width:38px;height:38px;border-radius:50%;object-fit:cover;">`
              : r.reviewer.name.charAt(0)}
          </div>
          <div style="flex:1;">
            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.5rem;margin-bottom:.25rem;">
              <div style="font-weight:700;font-size:.9rem;">${r.reviewer.name}</div>
              <div style="color:var(--amber);font-size:.9rem;">${'★'.repeat(r.rating)}${'☆'.repeat(5-r.rating)}</div>
            </div>
            ...
            <div style="font-size:.72rem;color:var(--text3);margin-top:.35rem;">${_formatRelDate(r.createdAt)}</div>
          </div>
        </div>
      </div>`).join('');
  } catch(e) {
    if (listEl) listEl.innerHTML = '<p style="color:var(--text2);padding:.5rem;">Ошибка загрузки отзывов</p>';
  }
}
...
async function saveEmailNotif() {
  try {
    const emailNotifications = document.getElementById('emailNotifToggle')?.checked;
    await apiAuth('/users/profile/update', {
      method: 'PUT',
      body: JSON.stringify({ emailNotifications })
    });
    showToast(emailNotifications ? 'Email уведомления включены 📧' : 'Email уведомления отключены', 'success');
...
async function saveTelegram() {
  ...
  if (!/^\d+$/.test(chatId)) { showToast('Chat ID должен состоять из цифр', 'error'); return; }
  try {
    await apiAuth('/users/profile/update', {
      method: 'PUT',
...
// Функция для profile.html — превью навыков при вводе
function updateSkillsPreview() {
  ...
  preview.innerHTML = skills.map(s=>`<span class="skill-tag">${s}</span>`).join('');
}